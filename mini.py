...
def main():
    try:
        ...
        def fetch_url_with_proxy(url, proxies, headers, max_retries=15):
        ...
        def getVacancyLinks(pagesLinks):
            ...
            for pageLink in pagesLinks:
                ...
                response = fetch_url_with_proxy(pageLink, proxies, headers)
                html = response.text
                soup = bs(html, "html.parser")
        ...
        def getVacancies(links):
            ...
            for index, link in enumerate(links, start=1):
                try:
                    ...
                    print(f"Задержка на {delay:.2f} секунд")
                    time.sleep(delay)
                    response = fetch_url_with_proxy(link, proxies, headers)
                    i = 0
                    if response.status_code == 200:
                        pass
                    html = response.text
